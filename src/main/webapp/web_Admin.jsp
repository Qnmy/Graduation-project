...
<html>
  <head>
    ...
    <title>My JSP 'web_Admin.jsp' starting page</title>
    ...
    <script type="text/javascript">
    	$(document).ready(function(){
    		$('#dg').datagrid({url:'web_AdminGet.action'});
    	});	
    </script>
...
<body>
    <h2>管理员信息</h2>
    <p>可进行管理员添加、修改、删除相关操作</p>
    <!-- <a href="b.html">转发</a>  --> <!--  url="web_AdminGet.action" -->
    <table id="dg" title="管理员信息" class="easyui-datagrid" style="width:80%;height:60%"
    ...
    <script type="text/javascript">
        ...
        function editUser(){
            ...
            if (row){
                ...
                $('#fm').form('load',row);
                url = 'web_AdminUpdate.action?id='+row.id;
            }
        }
        function saveUser(){
            $('#fm').form('submit',{
        ...
        function destroyUser(){
            ...
            if (row){
                $.messager.confirm('确认','确定要删除选中条目?',function(r){
                    if (r){
                        $.get('web_AdminDelete.action',{id:row.id},function(result){
                        	var result = eval('('+result+')');
                            if (result.success){
                                $('#dg').datagrid('reload');    // reload the user data
                            } else {
