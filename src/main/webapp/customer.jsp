  ...
  <head>
    ...
    <script type="text/javascript">
    	$(document).ready(function(){
    		$('#dg').datagrid({url:'customer_get.action'});
    	});	
    </script>
...
<body>
    <h2>客户信息情况</h2>
    <p>可对客户信息进行添加、修改、删除操作</p>
     <!--  url="web_AdminGet.action" -->
    <table id="dg" title="客户信息" class="easyui-datagrid" style="width:80%;height:60%"
...
<!--                 <th field="alipaynumber" width="50">支付宝账户</th>
 -->            </tr>
        </thead>
    </table>
    <div id="toolbar">
    ...
    <script type="text/javascript">
        ...
        function editUser(){
            ...
            if (row){
                ...
                $('#fm').form('load',row);
                url = 'customer_update.action?id='+row.id;
            }
        }
        function saveUser(){
            $('#fm').form('submit',{
        ...
        function destroyUser(){
            ...
            if (row){
                $.messager.confirm('确认','确定要删除选中条目?',function(r){
                    if (r){
                        $.get('customer_delete.action',{id:row.id},function(result){
                        	var result = eval('('+result+')');
                            if (result.success){
                                $('#dg').datagrid('reload');    // reload the user data
                            } else {
